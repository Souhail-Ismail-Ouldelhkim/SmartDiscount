    ...
    : IIntegrationEventHandler<OrderStatusChangedToPaidIntegrationEvent>
{
    ...
    <div style='font-family: Arial, sans-serif; max-width: 600px; margin: auto;'>
        ...
        <div style='margin-top: 20px; text-align: right;'>
            <p style='margin: 5px 0;'>Sous-total : {(order.Total + order.DiscountAmount):F2} $</p>
            {(order.DiscountAmount > 0 ? $"<p style='margin: 5px 0; color: green;'>Réduction{(string.IsNullOrEmpty(order.PromoCode) ? "" : $" ({order.PromoCode})")} : -{order.DiscountAmount:F2} $</p>" : "")}
            <h3 style='color: #1a2b4a; margin: 10px 0 0 0;'>Total payé : {order.Total:F2} $</h3>
        </div>
